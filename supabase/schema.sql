...
alter table public.candidates add column if not exists candidate_number text not null default '';
...
alter table public.candidates enable row level security; alter table public.articles enable row level security; alter table public.banners enable row level security; alter table public.feedback enable row level security; alter table public.admin_users enable row level security;
...
create policy "public submits feedback" on public.feedback for insert to anon,authenticated with check(status='nova');
create policy "admins read feedback" on public.feedback for select to authenticated using(public.is_sul_na_urna_admin());
create policy "admins update feedback" on public.feedback for update to authenticated using(public.is_sul_na_urna_admin()) with check(public.is_sul_na_urna_admin());
create policy "admins delete feedback" on public.feedback for delete to authenticated using(public.is_sul_na_urna_admin());
